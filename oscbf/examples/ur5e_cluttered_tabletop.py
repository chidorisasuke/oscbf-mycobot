...
def interactive_sphere_tuning(env, robot, control_func):
    ...
    while True:
        ...
        # # ================== TAMBAHKAN PRINT INI ==================
        # if keys: # Cetak hanya jika ada event terdeteksi
        #     print(f"DEBUG: Keyboard events detected: {keys}")
        # # =======================================================
...
def main(control_method="torque", num_bodies=25):
    ...
    # A natural, folded starting pose for the UR5e
    ...
    try:
        ...
        while True:
            ...
            if robot_sphere_ids: # Hanya jika bola berhasil dibuat
                # Get the local offsets from the collision model
                local_sphere_positions_per_link = ur5e_collision_data["positions"]
                
                sphere_id_counter = 0
                # Iterate through each link of the robot
                for i in range(robot.num_joints): # Assumes num_joints is the number of moving links
                    # Get the world position and orientation of the link from PyBullet
                    # Note: PyBullet link indices correspond to joint indices
                    try:
                        link_state = env.client.getLinkState(env.robot, i, computeForwardKinematics=1)
                        link_world_pos = link_state[4]
                        link_world_orn = link_state[5]
                    except Exception as e:
                        # print(f"Could not get link state for link {i}, skipping.")
                        continue
                    
                    # Get the local sphere positions for this link
                    local_positions = np.asarray(local_sphere_positions_per_link[i])
                    if local_positions.ndim == 1: # Handle single sphere case
                        local_positions = local_positions.reshape(1, -1)

                    if local_positions.shape[0] == 0:
                        continue

                    # Transform the local sphere positions to the world frame
            ...
            """
            # Detailed logging for debugging sphere positions
            if (env.t - last_print_time) >= 0.5: # Log every 0.5 seconds
                ...
                if robot_collision_data_world.size > 0:
                    robot_sphere_positions = robot_collision_data_world[:, :3]
                    num_spheres_per_link = [len(p) for p in robot.collision_positions]
                    sphere_idx_start = 0
                    for i in range(robot.num_joints):
                        joint_origin = joint_transforms[i, :3, 3]
                        print(f"--- Link {i+1} (Joint {i}) ---")
                        print(f"  Joint Origin (World): {np.round(joint_origin, 4)}")

                        num_spheres = num_spheres_per_link[i]
                        if num_spheres == 0:
                            print("  No collision spheres.")
                            continue

                        sphere_idx_end = sphere_idx_start + num_spheres
                        link_spheres_pos = robot_sphere_positions[sphere_idx_start:sphere_idx_end]

                        for j in range(num_spheres):
                            sphere_pos = link_spheres_pos[j]
                            distance = np.linalg.norm(sphere_pos - joint_origin)
                            print(f"  Sphere {j+1} Pos (World): {np.round(sphere_pos, 4)}")
                            print(f"    Distance to Joint Origin: {distance:.4f} m")

                        sphere_idx_start = sphere_idx_end
                print("=" * 30)
                last_print_time = env.t
            """
        ...
        # To prevent errors when plotting after an abrupt exit, make sure all logs have the same length
