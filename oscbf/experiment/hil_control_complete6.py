...
class PinocchioDynamicsCalculator:
    ...
    def get_nullspace(self, q, J):
        ...
        try:
            ...
            A = J @ M_inv @ J.T + 1e-6 * np.eye(3) # Regularisasi kecil
            Lambda = np.linalg.pinv(A)
            J_bar = Lambda @ J @ M_inv
...
def compute_obstacle_torque(q, dyn, obstacles):
    ...
    for obs in obstacles:
        ...
        # Tambahan: Exponential scaling untuk smooth transition
        scale = np.exp(-5.0 * (d / obs.safe_dist - 0.5))  # Peak di d=0.5*safe_dist
        dU_dd *= scale
...
def generate_trajectory_preview(x_base, num_points=100):
    """
    Generate trajectory points untuk visualisasi static
    """
    t_samples = np.linspace(0, RUNTIME, num_points)
    trajectory_points = []
    
    for t in t_samples:
        x_d, _, _ = calculate_trajectory_point(t, x_base)
        trajectory_points.append(x_d)
...
def main():
    ...
    # Obstacle - AKTIFKAN UNTUK TESTING
    # obstacles = [
    #     SimpleObstacle(center=[0.45, -0.15, 0.35], safe_dist=0.12, k_obs=50.0),
    #     # SimpleObstacle(center=[0.35, 0.10, 0.30], safe_dist=0.10, k_obs=50.0),  # Tambah obstacle kedua jika perlu
    # ]
    obstacles = []
    ...
    # --- Visualisasi Obstacle di PyBullet ---
    obstacle_visual_ids = []
    for obs in obstacles:
        visual_shape_id = p.createVisualShape(
            shapeType=p.GEOM_SPHERE,
            radius=obs.safe_dist,
            rgbaColor=[1, 0, 0, 0.3]  # Merah transparan
        )
    ...
    print(f"✅ {len(obstacles)} obstacle(s) ditambahkan ke PyBullet")
    ...
    if args.mode == "real":
        ...
        posisi_tcp = np.array(tcp_pose[:3])
        # x_base = np.array(tcp_pose[:3])
        # X=0.45 (Depan), Y=0.0 (Tengah), Z=0.4 (Tinggi ideal)
        x_base = posisi_tcp
    else:
        # Tentukan posisi dasar imajiner untuk mode simulasi
        x_base = np.array([0.45, 0.0, 0.40])
    ...
    print(f"DEBUG: Posisi TCP {np.round(posisi_tcp,3)}")
    print("   (Lintasan akan dimulai dari titik ini)")

    # ================================================================
    # VISUALISASI TRAJECTORY PATH DI PYBULLET
    # ================================================================
    print("\n🎨 Menggambar Trajectory Path...")

    # Generate trajectory preview points
    traj_points = generate_trajectory_preview(x_base, num_points=100)

    # Gambar garis trajectory (Hijau seperti referensi)
    # Digambar SEKALI saja dengan lifeTime=0 (permanen)
    for i in range(len(traj_points) - 1):
        p.addUserDebugLine(
            lineFromXYZ=traj_points[i].tolist(),
            lineToXYZ=traj_points[i + 1].tolist(),
            lineColorRGB=[0, 1, 0],  # Hijau (Sesuai ur5e_dynamic_motion2)
            lineWidth=2.5,
            lifeTime=0  # 0 = permanent (tidak akan hilang)
        )

    # Tambahkan marker titik awal (Biru) dan akhir (Merah)
    p.addUserDebugLine(
        lineFromXYZ=traj_points[0].tolist(),
        lineToXYZ=(traj_points[0] + np.array([0,0,0.1])).tolist(), # Garis vertikal kecil
        lineColorRGB=[0, 0, 1], # Start = Biru
        lineWidth=4,
        lifeTime=0
    )
    
    # Sphere visual untuk target (Bola Merah Kecil yang bergerak)
    target_visual_shape = p.createVisualShape(p.GEOM_SPHERE, radius=0.015, rgbaColor=[1, 0, 0, 0.8])
    target_body_id = p.createMultiBody(baseVisualShapeIndex=target_visual_shape, basePosition=traj_points[0])

    print(f"✅ Trajectory digambar: {len(traj_points)} titik.")
    ...
    if args.mode == "real":
        ...
        target_pos = list(start_pose) # Pastikan format list untuk dimodifikasi
        target_pos[0] = x_start_traj[0]
        target_pos[1] = x_start_traj[1]
        target_pos[2] = x_start_traj[2]

        dist_to_move = np.linalg.norm(x_start_traj - np.array(start_pose[:3]))
        print(f"   📏 Jarak geser: {dist_to_move*1000:.1f} mm")
        
        print(f"   🚶 Robot berjalan ke: {np.round(x_start_traj, 3)} ... Mohon tunggu.")
        # Gerakkan robot pelan-pelan ke titik start
        rtde_c.moveL(target_pose, 0.1, 0.2) 
        time.sleep(0.5) # Beri waktu robot untuk stabil
        
        print("   ✅ Robot sampai di titik awal.")
    ...
    # Reset command ke posisi saat ini
    q_cmd = q_real.copy() 
    ...
    # Validasi Error Awal (Safety Check)
    err_awal = np.linalg.norm(x_start_traj - x_actual)
    print(f"   🔍 Error Awal: {err_awal*1000:.2f} mm")
    if err_awal > 0.05: # Jika error masih > 5cm, jangan lanjut (bahaya)
        print("❌ BAHAYA: Robot gagal mencapai titik awal presisi. Aborting.")
        return
    ...
    log_obstacle_dist = []  # Jarak ke obstacle terdekat
    log_obstacle_force = []  # Magnitude gaya repulsif
    ...
    try:
        while time.time() - start_time < RUNTIME:
            ...
            # --- Real-time trajectory trace (jejak yang sudah dilalui) ---
            if 'last_ee_pos' in locals() and t > 0.1:  # Mulai setelah 0.1 detik
                # Gambar garis dari posisi sebelumnya ke posisi sekarang
                trace_line = p.addUserDebugLine(
                    lineFromXYZ=last_ee_pos.tolist(),
                    lineToXYZ=x_actual.tolist(),
                    lineColorRGB=[1, 0.5, 0],  # Oranye untuk jejak aktual
                    lineWidth=2,
                    lifeTime=RUNTIME * 2  # Bertahan sampai akhir
                )

            # Update posisi terakhir
            last_ee_pos = x_actual.copy()
                        # Update posisi target visual (Bola Merah) mengikuti trajectory
            p.resetBasePositionAndOrientation(target_body_id, x_des.tolist(), [0,0,0,1])

            # --- Display Info (Tanpa removeAllUserDebugItems) ---
            if int(t * 10) % 5 == 0:  # Update setiap 0.5 detik
                # HAPUS baris ini: p.removeAllUserDebugItems() 
                
                # Gunakan lifeTime pendek agar text hilang sendiri tanpa menghapus garis hijau
                info_text = f"Time: {t:.2f}s | Traj: {TRAJECTORY}"
                p.addUserDebugText(info_text, [x_base[0], x_base[1], x_base[2] + 0.3], 
                                   textColorRGB=[0,0,0], textSize=1.2, lifeTime=0.6)
                
                # Visualisasi Error (Text saja)
                e_pos = x_des - x_actual
                error_norm = np.linalg.norm(e_pos)
                p.addUserDebugText(f"Error: {error_norm*1000:.1f}mm", 
                                   [x_base[0], x_base[1], x_base[2] + 0.25],
                                   textColorRGB=[1,0,0] if error_norm > 0.01 else [0,0.5,0],
                                   textSize=1.0, lifeTime=0.6)

                # Update warna obstacle (jika ada)
                for i, obs in enumerate(obstacles):
                    dist = np.linalg.norm(x_actual - obs.center)
                    color = [1, 0, 0, 0.8] if dist < obs.safe_dist else [1, 0, 0, 0.3]
                    p.changeVisualShape(obstacle_visual_ids[i], -1, rgbaColor=color)
            ...
            # Logging obstacle
            if len(obstacles) > 0:
                min_dist = min([np.linalg.norm(x_actual - obs.center) for obs in obstacles])
                obs_force_mag = np.linalg.norm(tau_null)  # Magnitude torsi null space
                log_obstacle_dist.append(min_dist)
                log_obstacle_force.append(obs_force_mag)
            else:
                log_obstacle_dist.append(np.nan)
                log_obstacle_force.append(np.nan)
        ...
        if len(log_t) > 0:
            ...
            # Plot Obstacle Avoidance Metrics
            if len(obstacles) > 0 and len(log_obstacle_dist) > 0:
                fig_obs, axes_obs = plt.subplots(2, 1, figsize=(12, 8))
                
                # Plot jarak ke obstacle
                axes_obs[0].plot(log_t, log_obstacle_dist, 'b-', linewidth=2, label='Distance to Closest Obstacle')
                axes_obs[0].axhline(y=obstacles[0].safe_dist, color='r', linestyle='--', label='Safe Distance')
                axes_obs[0].fill_between(log_t, 0, obstacles[0].safe_dist, alpha=0.2, color='red', label='Danger Zone')
                axes_obs[0].set_ylabel('Distance (m)')
                axes_obs[0].set_title('Obstacle Avoidance - Distance')
                axes_obs[0].grid(True, alpha=0.5)
                axes_obs[0].legend()
                
                # Plot magnitude gaya repulsif
                axes_obs[1].plot(log_t, log_obstacle_force, 'g-', linewidth=2, label='Repulsive Force Magnitude')
                axes_obs[1].set_xlabel('Time (s)')
                axes_obs[1].set_ylabel('Force (Nm)')
                axes_obs[1].set_title('Obstacle Avoidance - Repulsive Torque')
                axes_obs[1].grid(True, alpha=0.5)
                axes_obs[1].legend()
                
                fig_obs.tight_layout()
                filename_obs = f"HIL_ObstacleAvoidance_{TRAJECTORY}_mode_{args.mode}_speed_{args.speed}_fitur_{args.fitur}_nullspace_{args.nullspace}_speedMode_{args.speed_mode}_command_{args.command}_{timestamp}.png"
                plt.savefig(os.path.join(folder_name, filename_obs), dpi=300, bbox_inches='tight')
                print(f"💾 Disimpan: {os.path.join(folder_name, filename_obs)}")
